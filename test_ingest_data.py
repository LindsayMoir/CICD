...
def test_ingest_data_negative_feature_1():
    ...
    response = client.post("/ingest_data", json={"feature_1": -5, "feature_2": "Valid string"})
...
def test_ingest_data_long_feature_2():
    ...
    response = client.post("/ingest_data", json={"feature_1": 10.5, "feature_2": long_string})
    assert response.status_code == 400
    assert response.json() == {"detail": "feature_2 must be 280 characters or less"}

def test_ingest_data_valid_input():
    # Test with valid input
    response = client.post("/ingest_data", json={"feature_1": 10.5, "feature_2": "Valid string"})
